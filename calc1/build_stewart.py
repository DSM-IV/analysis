...
expected={}
for inventory in (R/'stewart/exercise-checks').glob('calc1-ch*-inventory.json'):
 for section in json.loads(inventory.read_text())['sections']:
  expected[section['section']]=section.get('total',section.get('exerciseCount'))
completed={r['section']:len(r['completedNumbers']) for r in rows}
complete=bool(expected) and all(completed.get(section)==total for section,total in expected.items())
status='1–13장 일반 Exercises 전체의 문제와 해설을 제공합니다.' if complete else '나머지 절은 작성·검산을 마치는 대로 추가합니다.'
cards=[]
for r in rows:
...
<header><p class="subtitle">Stewart · Calculus · Ninth Edition</p><h1>미적분학 1 · 교재 연습문제<span>Exercises and Solutions</span></h1><p class="header-desc">1–13장의 일반 Exercises를 절별로 제공합니다. 문제 요약, 힌트, 단계별 풀이와 검산을 한국어·영어로 읽을 수 있습니다.</p><p class="header-desc">현재 공개: COUNT문항 · STATUS</p></header>
...
'''.replace('COUNT',f'{count:,}').replace('STATUS',status).replace('CARDS','\n'.join(cards))
