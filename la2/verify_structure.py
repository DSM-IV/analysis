#!/usr/bin/env python3
"""la2 structure verifier (stdlib only). Usage: python3 la2/verify_structure.py [files...]"""
...
def check(path):
    ...
    r['cards'] = len(re.findall(r'<div class="card (?:def|thm|rem|exam|ex)-card" id="c\d+-\d+"', s))
    r['ex_cards'] = len(re.findall(r'<div class="card ex-card" id="(?:c\d+-\d+|e\d+-\d+)"', s))
    ...
    # banned terms
    banned = ['선형결합', '선형독립', '선형종속', '고윳값', '특성값', '고유다항식', '대각화 가능']
